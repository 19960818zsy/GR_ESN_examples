...
noiselevel = 0.001;
linearOutputUnits = 1; % 1 = use linear output units, 0 = sigmoid output units
...
intWM = setPrecision(intWM0 * specRad, no_values, interpol);
ofbWM = setPrecision(ofbWM0 * diag(ofbSC), no_values, interpol);
outWM = setPrecision(initialOutWM, no_values, interpol);
% intWM = intWM0 * specRad, no_values, interpol;
% ofbWM = ofbWM0 * diag(ofbSC), no_values, interpol;
% outWM = initialOutWM, no_values, interpol;
...
for i = 1:initialRunlength + sampleRunlength + freeRunlength + plotRunlength 
    ...
    if i == initialRunlength + sampleRunlength
        if WienerHopf
            ...
            outWM = setPrecision((inv(covMat) * pVec)', no_values, interpol);
            %outWM = (inv(covMat) * pVec)';
        else
            %outWM = setPrecision((pinv(stateCollectMat) * teachCollectMat)', no_values, interpol);
