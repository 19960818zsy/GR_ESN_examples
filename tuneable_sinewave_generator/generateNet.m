...
while not(success)
    try
intWM0 = sprand(netDim, netDim, connectivity); % randomly generating internal weight matrix
intWM0 = spfun(@minusPoint5,intWM0);
opts.disp = 0;
specRad = abs(eigs(intWM0,1, 'lm', opts)); % computing the spectral radius of the randomly generated internal weight matrix
intWM0 = intWM0/specRad; % normalizig the internal weight matrix to unit spectral radius
success = 1;
    catch
        ;
    end
