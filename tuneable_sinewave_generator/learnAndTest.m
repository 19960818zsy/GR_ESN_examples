...
linearNetwork = 0; % 1 = use linear units in DR, 0 = use tanh units
...
%initialRunlength = 200;

% number of update steps used for computing output weights
%sampleRunlength = 7000;
sampleRunlength = 2700;
...
totalstate =  zeros(totalDim,1); % totalDim = netDim + inputLength + outputLength; -> set in generateNet    
internalState = totalstate(1:netDim);
intWM = intWM0 * specRad; % scaling the internal weight matrix W to a spectral radius < 1 (set above)
ofbWM = ofbWM0 * diag(ofbSC); % scaling the output feedback weight matrix
outWM = initialOutWM; % initializing the output weight matrix
    ...
    if i == initialRunlength + sampleRunlength
        if WienerHopf
            ...
            if Pinv
                ...
                outWM = teachCollectMat'*stateCollectMat*pinv(stateCollectMat'*stateCollectMat + (TychonovReg^2)*eye(size(stateCollectMat,2)));
            end
        end
